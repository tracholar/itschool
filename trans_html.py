...
def trans_html(T, xpath=None):
    ...
    for i, x in enumerate(xs):
        if x.tag in ('p', 'li', 'td', 'dt', 'blockquote', 'h1', 'h2', 'h3', 'h4', 'h5' , 'h6', 'aside') or (x.tag is 'div' and len(x.text.strip())>10) :
            html = dom_to_html(x)
            ...
            ts_tag_s[i] = x.tag
        elif x.tag in ('div', 'section', 'table', 'ul', 'ol', 'dl', 'dd' ,'tr', 'tbody', 'thead', 'article', 'header', 'footer'):
